class AVLNode:
    ...
    def is_root(self): return isinstance(self.parent, Tree)
    ...
    @staticmethod
    def _kthLrgest(root, k, c):
        if root is None or c[0]>=k:
            return 'none'
        left = AVLNode._kthLrgest(root.left, k, c)
        if left != 'none':
            return left
        c[0] += 1
        if c[0] == k:
            return root.key
        return AVLNode._kthLrgest(root.right, k, c)

    @staticmethod
    def _next(root, x):
        if root is None or (root.key == x and not root.has_right()):
            return 'none'
        elif root.key > x:
            k = AVLNode._next(root.left, x)
            if k == 'none':
                return root.key
            else:
                return k
        elif root.key < x:
            return AVLNode._next(root.right, x)
        elif root.key == x:
            return AVLNode._next(root.right, x)

    @staticmethod
    def _prev(root, x):
        if root is None or (root.key == x and not root.has_left()):
            return 'none'
        elif root.key < x:
            k = AVLNode._prev(root.right, x)
            if k == 'none':
                return root.key
            else:
                return k
        elif root.key > x:
            return AVLNode._prev(root.left, x)
        elif root.key == x:
            return AVLNode._prev(root.left, x)
...
class Tree:
    ...
    def next(self, x):
        if not self.is_empty():
            return AVLNode._next(self.root, x)
        else:
            return 'none'

    def prev(self, x):
        if not self.is_empty():
            return AVLNode._prev(self.root, x)
        else:
            return 'none'
    ...
    def kth(self, k):
        c = [0]
        if not self.is_empty():
            return AVLNode._kthLrgest(self.root, k, c)
        else:
            return 'none'
...
while True:
    ...
    if st == 'insert':
        ...
        print(root.next(int(x)))
    elif st == 'prev':
        print(root.prev(int(x)))
    elif st == 'kth':
        print(root.kth(int(x)))
