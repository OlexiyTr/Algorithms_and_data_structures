...
def __count_helper(graph, visited, start, end, count, dist = 0):
    visited.add(start)
    if start == end and dist <= d:
        count[0] += 1
    else:
        for neighbour in graph[start].neighbors():
            if neighbour not in visited and dist<= d:
                __count_helper(graph, visited, neighbour, end, count, dist + 1)
...
def countPaths(graph, start, end):
    visited = set()
    count = [0]
    __count_helper(graph, visited, start, end, count)
    return count[0]
