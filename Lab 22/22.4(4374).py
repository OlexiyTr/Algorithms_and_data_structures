...
def isBridge(graph, C):
    for el in C:
        del graph[inp[el-1][0]].mNeighbors[inp[el-1][1]]
        del graph[inp[el-1][1]].mNeighbors[inp[el-1][0]]

    res = __check_connected_helper(graph, 1)

    for el in C:
        graph[inp[el-1][0]].mNeighbors[inp[el-1][1]] = 1
        graph[inp[el-1][1]].mNeighbors[inp[el-1][0]] = 1
...
for i in range(k):
    ...
    try:
        print('Connected' if  isBridge(graph, C) else 'Disconnected')
    except:
        print('Disconnected')
